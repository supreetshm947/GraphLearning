...
def get_wl_embedding(g: nx.Graph, num_rounds: int = 4):
    recolor_node(g)
...
def recolor_node(g: nx.Graph):
    ...
    for node_label in node_labels:
        if 'multiset' in node_label[1]:
            val = abs(hash(str(node_label[1]['multiset'])))
        else:
            val = abs(hash(str(node_label[1]['node_label'])))
...
def get_wl_embeddings(graphs):
    distinct_color = set()
    col_graphs = list()
    for i in graphs:
        col_graphs.append(get_wl_embedding(i))
...
get_wl_embeddings(nx.read_gpickle("../datasets/datasets/DD/data.pkl"))
